...
Inductive uvector (X: Type) : Type :=
| mku (n: nat) (v: @vector X n) : uvector X
.

Definition ulength {X} (u: uvector X) : nat :=
  match u with
  | mku _ n _ => n
  end.

Definition vlength {X} (n: nat) (v: @vector X n) : nat := n.

(* uvector X -> nat *)

Check @vnil nat.
Check (vcons 3 vnil).
Check (vcons 1 (vcons 3 vnil)).

Check @vector.

Check { n & vector n }.

Check existT _ 1 (vcons 3 vnil) : { n & vector n }.
