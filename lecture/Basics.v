...
(* (fix foo n := foo n : nat) 0 *)
(* -> *)
(* (fix foo n := foo n : nat) 0   *)

(* Definition foo := (fix foo n := foo n : nat). *)
...
Definition plus_n_O: forall (n : nat), n + 0 = n :=
  fix self n :=
     ...
     | S n0 =>
         match
           match self n0 in (_ = y) return (y = n0 + 0) with
           | eq_refl => eq_refl
           end in (_ = y) return (S y = S n0)
         with
         | eq_refl => eq_refl
         end
     end.
