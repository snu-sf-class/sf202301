...
Fixpoint applyn (f: nat -> nat) i n :=
  match i with
  | 0 => n
  | S i' => applyn f i' (f n)
  end.

Definition reaches'_1 (n:nat) := exists i, applyn f i n = 1.
...
| le_n (n : nat): le n n
| le_S: forall (n m : nat) (LE: le n m), le n (S m)
.
...
Inductive clos_trans {X: Type} (R: X->X->Prop) : X->X->Prop :=
  | t_step x y
    ...
    clos_trans R x y
  | t_trans x y z
      (RELl: clos_trans R x y)
      (RELr: clos_trans R y z):
    clos_trans R x z
.
...
(** We could try to proceed by case analysis or induction on [n].  But
  ...
  intros n.
  assert (STR: forall m: nat, m <= n -> ev m -> exists n0 : nat, m = double n0).
  {
    induction n.
    - intros. inversion H.
    ...
    - intros m LE EV.
      destruct EV.
      + exists 0. eauto.
      + (* specialize (IHn n0). edestruct IHn. *)
        (* edestruct (IHn n0); eauto; try nia. *)
        hexploit (IHn n0); eauto; try nia.
        intros [x DBL].
        exists (S x). rewrite DBL. eauto.
  }
  apply (STR n); eauto.

(*  
  induction n.
  - intros. eexists 0. simpl. eauto.
  - intros EV.
...
Proof.
  ...
  - (* <- *) unfold Even. intros [k Hk].
    rewrite Hk. clear n Hk. apply ev_double.
Qed.
