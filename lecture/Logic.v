...
Check (forall P Q, P \/ Q).
Check (forall P Q, P -> Q: Prop).
Check (forall P: Prop, P -> False).
Check (forall P: Prop, P -> True).
Check (forall (P: nat -> Prop) (n: nat), P n).
Check (forall (P: nat -> Prop), exists n, P n).

(* Lemma fermat_last : (forall n: nat, n >= 3 -> (exists a b c: nat, exp a n + exp b n = exp c n) -> False). *)
(* Proof. *)
(* Qed. *)
...
Check ((@ex_intro _ _ 2 eq_refl) : @ex nat (fun n => n = n*n - n)).
Check ((@ex_intro _ _ 2 eq_refl) : exists n, n = n*n - n).

(*
exists n, n = (n*n - n)      ====   ex _ (fun n => n = n*n - n)
exists n:nat, n = (n*n - n)  ====   ex nat (fun n => n = n*n - n)
                                             
exists n:nat, e  ====  ex nat (fun n => e)
                 === {(3, pf: P 3), ... }
*)            
...
Proof.
  ...
  - (* Here, [n = 0] *)
    rewrite Hn. simpl. reflexivity.
  - (* Here, [m = 0] *)
    rewrite Hm. simpl. rewrite <- mult_n_O.
    reflexivity.
Qed.
