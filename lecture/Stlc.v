...
Inductive tm : Type :=
  ...
  | tm_app   : tm -> tm -> tm
  | tm_abs   : string -> ty -> tm -> tm (* tm_abs x ty tm =>  fun x : ty => tm *)
...
Check <{ (\x: Bool, if x then false else true) false }>.
...
(* a non-sense case *)
Compute subst x <{y}> <{\y: Bool->Bool, y x}>.
